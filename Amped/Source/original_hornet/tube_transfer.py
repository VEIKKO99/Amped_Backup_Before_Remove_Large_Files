...
class Circuit(object):
    ...
    def set_param(self):
        ...
        if (self.ipk_func == "pentode" or self.ipk_func == "pentode2"):
    ...
    def Ftube(self, Vi, Ri):
        ...
        """
        def fi(Vgk, Vi, Ri):
            return Vi - Vgk - (Ri +self.rgi) * self.Igk_Vgk(Vgk) # sum of voltages -> zero

        Vgk = newton(fi, self.Igk_Vgk(0), args=(Vi, Ri), maxiter=100000)   # Vgk(Vi)

        def fp(Vpk, Vgk, Ipk):
            return Vpk + self.Rp * Ipk(Vgk, Vpk) - self.Vp

        return newton(fp, self.Vp/2, args=(Vgk,self.Ipk), maxiter=100000)   # Vpk(Vgk)
    ...
    def s_plot_Ftube(self):
        ...
        for Ri in self.Ri_values:
            Vp = self.FtubeV(self.Vi, Ri)
            plot(self.Vi, Vp, label="Ri=%dk \n Rk 2700 Vk0=%f\n Rk 1500 Vk0=%f\n Rk  820 Vk0=%f\n Rk" % ((Ri/1e3), self.Vk0(Ri,2700.0), self.Vk0(Ri,1500.0), self.Vk0(Ri,820.0)))
        xlabel("Vik")
        ylabel("Vp")
        legend()
        axis
